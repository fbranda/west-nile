...
# Read data
# Human
dat_wn <- map_dfr(links[grep("umana", links)], 
  ...
  summarise(nuovi_casi = sum(nuovi_casi, na.rm = T)) %>% ungroup()
...
data_model <- dat_wn %>% 
  ...
  rename(ti_orig = data, region = denominazione_regione) %>% 
  group_split()
...
fit_richards <- function(dat, fam = "Negative Binomial", horizon = 0, off = 1, Xcov = NULL){
  ...
  whidx <- which(colnames(dat)=="nuovi_casi")
  ...
  richout <- fitTib %>% 
    full_join(dat %>% dplyr::select(ti_orig,  "nuovi_casi"), by="ti_orig") %>% 
...
############# VENETO #############
# 2022
veneto_2022 <- data_model[[which(regY$denominazione_regione == "Veneto" & regY$Y == 2022)]] %>% filter(ti_orig<="2022-09-20")

c(np, richout, parsout) %<-% fit_richards(dat = veneto_2022, fam = "Poisson", off = 4847147/1000, horizon = 3)

veneto_2022_last <- data_model[[which(regY$denominazione_regione == "Veneto" & regY$Y == 2022)]]  %>% 
  filter(ti_orig %in% richout$x1[(nrow(veneto_2022)+1):(nrow(veneto_2022)+3)]) 

ggplot() +
  geom_point(data = richout, aes(x = x1, y = pc), size = 4) +
  geom_point(data = veneto_2022_last, aes(x = ti_orig, y = nuovi_casi), size = 4, shape = 18, color = "red") +
...
np$R2diff
sqrt(mean((richout$y[(nrow(richout)-2):nrow(richout)] - veneto_2022_last$nuovi_casi)^2))


# 2018
veneto_2018 <- data_model[[which(regY$denominazione_regione == "Veneto" & regY$Y == 2018)]]
...
############# EMILIA-ROMAGNA #############
# 2018
ER_2018 <- data_model[[which(regY$denominazione_regione == "Emilia-Romagna" & regY$Y == 2018)]]
...
# 2022
  ER_2022 <- data_model[[which(regY$denominazione_regione == "Emilia-Romagna" & regY$Y == 2022)]] %>% filter(ti_orig<="2022-09-20")

c(np, richout, parsout) %<-% fit_richards(dat = ER_2022, fam = "Poisson", off = 4458006/1000, horizon = 3)

ER_2022_last <- data_model[[which(regY$denominazione_regione == "Emilia-Romagna" & regY$Y == 2022)]]  %>% 
  filter(ti_orig %in% richout$x1[(nrow(ER_2022)+1):(nrow(ER_2022)+3)]) 

ggplot() +
  geom_point(data = richout, aes(x = x1, y = pc), size = 4) +
  geom_point(data = ER_2022_last, aes(x = ti_orig, y = nuovi_casi), size = 4, shape = 18, color = "red") +
  geom_line(data = richout, aes(x = x1, y = y), color = "red", linewidth = 1.01) +
  geom_ribbon(data = richout, aes(x = x1, ymin = ly, ymax = uy), alpha = .4, fill = I("salmon")) +
  labs(x = "", y = "West nile cases") +
  theme_bw() +
